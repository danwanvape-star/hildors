...
void main() {
  test(
      'legacy or unspecified creator visibility falls back without exposing identity',
      () {
    final legacy = record('legacy')..remove('creator');
    final value = RemoteCatalogPackage.fromJson(legacy);
    expect(value.credit, '匿名创作者');
    expect(value.hasPublicCreator, isFalse);
    final ambiguous = RemoteCatalogPackage.fromJson({
      ...record('hidden'),
      'creator': {'id': 'private-id', 'name': 'Private'}
    });
    expect(ambiguous.credit, '匿名创作者');
    expect(ambiguous.creatorId, isNull);
    expect(ambiguous.creatorName, isNull);
  });
  testWidgets(
      'official credit belongs to its content card, not a source heading',
      (tester) async {
            ...
            body: RemoteCatalogPage(
                load: () async => [
                      RemoteCatalogPackage.fromJson(
                          {...record('official'), 'source': 'hildors'})
                    ]))));
    await tester.pumpAndSettle();
    final credit = find.byKey(const ValueKey('credit-official'));
    expect(tester.widget<Text>(credit).data, 'HILDORS 出品');
    expect(
        find.ancestor(of: credit, matching: find.byType(Card)), findsOneWidget);
