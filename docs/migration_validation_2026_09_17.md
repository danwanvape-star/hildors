...
从原始归档重新解包到 D:\HildorsBackups\20260917T074954Z\workflow-restore-check，与前次读取测试副本分离。以临时凭据和本机回环端口启动，创建 RESTORE-TEST-ONLY 标记的测试记录，使用项目已有 showcase_01.mp4 验证上传、SHA-256、实际解码和缩略图生成、隔离测试审核、发布、发布后服务重启持久化及下架。测试审核标记 TEST-ONLY-NOT-A-LICENSE，不构成真实授权或线上审核。

各步骤通过；草稿及下架后的公开详情不可见，下架后预览返回 404，发布时 hardwareReady 仍为 false。逐一比较恢复副本原有记录，均未变化。测试服务已停止；未操作线上数据或服务，也没有把数据库或媒体写入 Git。报告保存在该备份目录 workflow-restore-verification.json。

恢复验证现已覆盖启动、鉴权、媒体读取解码与内容写入生命周期；尚不代表浏览器视觉或真机/P20 验收，也未做生产故障切换。周期性停机备份的时段与保留方案仍需确认后配置。
