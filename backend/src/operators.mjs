...
export const operatorActorContext = new AsyncLocalStorage();
...
export function operatorOperations(db){
 ...
 const decode=r=>r?{id:r.id,username:r.username,displayName:r.display_name,active:!!r.active,permissions:JSON.parse(r.permissions),version:r.version,createdAt:r.created_at,updatedAt:r.updated_at}:null;
 ...
 return {
  ...
  createOperator(value,actor){const username=String(value?.username??'').trim().toLowerCase();if(!/^[a-z0-9_.-]{3,64}$/.test(username)||typeof value.displayName!=='string'||!value.displayName.trim()||value.displayName.length>80||value.active!==undefined&&typeof value.active!=='boolean')throw new Error('OPERATOR_INVALID');const keys=permissions(value.permissions);const hash=passwordHash(value.password);if(db.prepare('SELECT 1 FROM operators WHERE username=?').get(username))throw new Error('OPERATOR_USERNAME_EXISTS');const id=randomUUID(),now=new Date().toISOString();db.prepare('INSERT INTO operators VALUES (?,?,?,?,?,?,1,1,?,?)').run(id,username,value.displayName.trim(),hash,value.active===false?0:1,JSON.stringify(keys),now,now);audit(actor,'operator.create',id,keys);return decode(get(id));},
  updateOperator(id,value,actor){const r=get(id);if(!r)throw new Error('OPERATOR_NOT_FOUND');if(value.version!==r.version)throw new Error('CONFLICT');const keys=permissions(value.permissions??JSON.parse(r.permissions));if(actor?.id===id&&keys.some(k=>!actor.permissions.includes(k)))throw new Error('OPERATOR_SELF_ESCALATION');const name=value.displayName??r.display_name,active=value.active??!!r.active;if(typeof name!=='string'||!name.trim()||name.length>80||typeof active!=='boolean')throw new Error('OPERATOR_INVALID');db.prepare('UPDATE operators SET display_name=?,active=?,permissions=?,version=version+1,auth_version=auth_version+1,updated_at=? WHERE id=?').run(name.trim(),+active,JSON.stringify(keys),new Date().toISOString(),id);audit(actor,'operator.update',id,keys);return decode(get(id));},
  resetOperatorPassword(id,value,actor){const r=get(id);if(!r)throw new Error('OPERATOR_NOT_FOUND');if(value.version!==r.version)throw new Error('CONFLICT');db.prepare('UPDATE operators SET password_hash=?,version=version+1,auth_version=auth_version+1,updated_at=? WHERE id=?').run(passwordHash(value.password),new Date().toISOString(),id);audit(actor,'operator.password',id);return decode(get(id));},
