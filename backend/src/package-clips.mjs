...
export const visiblePackage = item => !item?.deletedAt && item?.status === 'published' && item.clips.some(visibleClip);
...
export const editableClip = (item, clip) => !item?.deletedAt && Boolean(clip) && (item?.status === 'draft'
