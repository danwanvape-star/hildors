...
export async function coverThumbnail(directory, cover) {
  ...
  const target = resolve(directory, 'cover-thumbnails', `${cover.id}.jpg`);
  ...
  const work = (async () => {
    ...
    return bounded(async () => {
      ...
      await mkdir(resolve(directory, 'cover-thumbnails'), { recursive: true });
      ...
      try {
        await run('ffmpeg', ['-nostdin', '-hide_banner', '-loglevel', 'error', '-max_alloc', '134217728',
          ...
          '-vf', "scale=w='min(384,iw)':h='min(384,ih)':force_original_aspect_ratio=decrease",
          '-threads', '1', '-q:v', '4', '-f', 'image2', '-vcodec', 'mjpeg', '-update', '1', temporary]);
        const output = await stat(temporary);
        if (output.size < 4 || output.size > 1024 * 1024) throw new Error('INVALID_IMAGE');
...
export async function serveCoverThumbnail(req, res, directory, cover, preflight) {
  const path = await coverThumbnail(directory, cover);
  const bytes = await readFile(path);
  if (preflight && !preflight()) return;
  const etag = `"cover-${cover.id}-384-v1"`;
