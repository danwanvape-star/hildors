...
async function identity() {
  ...
  canEdit = ['orders.review','orders.quote','orders.deliver'].some(can);
}
...
function render(order) {
  ...
  const unpaidReview = canEdit && ['free_review', 'needs_info', 'approved_for_quote', 'quoted'].includes(order.status) && payment?.status !== 'purchased' && payment?.status !== 'refunded';
  ...
  if ((unpaidReview && can('orders.review')) || (order.status === 'quality_review' && can('orders.deliver'))) {
    ...
    if (unpaidReview && can('orders.review')) form.append(actionButton('要求补充资料', 'request_info', true), actionButton('拒绝申请', 'reject', true));
    if (can('orders.deliver') && order.status === 'quality_review' && payment?.status === 'purchased') form.append(actionButton('质检通过，交用户验收', 'approve_delivery', false), actionButton('退回制作修改', 'request_rework', true));
    section.append(form);
  }
...
$('refresh').onclick = () => run(async()=>{await identity();await load();}, '正在读取订单…');
$('previous').onclick = () => { if (page > 1) { page--; run(load); } };
$('next').onclick = () => { if (hasNext) { page++; run(load); } };
run(async()=>{await identity();await load();}, '正在读取订单…');
