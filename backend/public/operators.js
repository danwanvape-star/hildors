...
function canAdmin(permission) { return Boolean(adminActor) && !adminActor.mustChangePassword && (adminActor.isRoot === true || adminPermissionSet.has(permission)); }
function applyAdminPermissions() {
  if ($('own-password-open')) $('own-password-open').hidden = !adminActor || adminActor.isRoot === true;
...
function clearOperatorManagement() {
  ...
  operatorSaving = false; closeOperatorEditor(); clearOwnPassword(); if ($('own-password-dialog')?.open) $('own-password-dialog').close();
...
function renderOperators() {
  ...
  for (const item of operatorItems) {
    ...
    row.append(name, text('td', item.active ? (item.mustChangePassword ? '待本人改密' : '启用中') : '已停用'), text('td', `${item.permissions.length} 项权限`), actions); body.append(row);
  }
...
let ownPasswordSaving = false;
function clearOwnPassword() {
  for (const id of ['own-current-password','own-new-password','own-confirm-password']) if ($(id)) $(id).value = '';
}
function openOwnPassword() {
  if (!adminActor || adminActor.isRoot || ownPasswordSaving) return;
  clearOwnPassword(); $('own-password-error').textContent = '';
  $('own-password-hint').textContent = adminActor.mustChangePassword ? '首次登录或管理员重置密码后，请先设置本人密码，再重新登录。' : '修改后所有已登录会话失效，请使用新密码重新登录。';
  if (!$('own-password-dialog').open) $('own-password-dialog').showModal();
}
async function saveOwnPassword(event) {
  event.preventDefault(); if (!adminActor || adminActor.isRoot || ownPasswordSaving) return;
  const password = $('own-new-password').value;
  if (password.length < 12 || password.length > 128 || password !== $('own-confirm-password').value) { $('own-password-error').textContent = '新密码须为 12–128 个字符，两次输入必须一致。'; return; }
  const identity = adminIdentityRequest, body = {currentPassword:$('own-current-password').value,password};
  ownPasswordSaving = true; $('own-password-save').disabled = true;
  try {
    await api('/admin/me/password',body);
    if (identity !== adminIdentityRequest) return;
    endAdminSession();
    $('notice').textContent = '密码已修改，请使用新密码重新登录。';
  } catch(error) { if (identity === adminIdentityRequest) $('own-password-error').textContent = error.message; }
  finally { delete body.currentPassword; delete body.password; clearOwnPassword(); ownPasswordSaving = false; $('own-password-save').disabled = false; }
}
function initOwnPassword() {
  if (!$('own-password-form')) return;
  $('own-password-open').onclick = openOwnPassword;
  $('own-password-form').onsubmit = saveOwnPassword;
  $('own-password-close').onclick = () => { if (!ownPasswordSaving) {clearOwnPassword(); $('own-password-dialog').close();} };
  $('own-password-dialog').addEventListener('cancel',event => {if (ownPasswordSaving) event.preventDefault(); else clearOwnPassword();});
  $('own-password-dialog').addEventListener('close',clearOwnPassword);
}
