...
if (typeof document !== 'undefined') {
  ...
  async function identity() {
    permissions = []; canEdit = false; list.replaceChildren();
    const data = await request('/admin/me'); permissions = data.permissions ?? [];
    canEdit = ['plans.edit','plans.pricing','plans.publish','plans.billing'].some(can);
  }
  const el = (tag, text = '') => {
  ...
  async function run(work, status) {
    ...
    try { await work(); } catch (error) { message.textContent = error.message; }
      ...
      busy = false;
      document.querySelectorAll('button, input').forEach(control => { control.disabled = control.dataset.permission ? !can(control.dataset.permission) : control.tagName==='INPUT' || control.type==='submit' ? !canEdit : false; });
    }
  }
  ...
  function render(plan) {
    ...
    for (const input of [duration,en,zh,order]) input.dataset.permission = 'plans.edit';
    for (const input of [price,markup]) input.dataset.permission = 'plans.pricing';
    for (const input of [apple,google,audioApple,audioGoogle]) input.dataset.permission = 'plans.billing';
    listed.dataset.permission = 'plans.publish';
    const save = el('button', '保存此套餐');
    save.type = 'submit';
    form.append(save);
    form.onsubmit = event => {
      event.preventDefault();
      if (!canEdit || busy || !form.reportValidity()) return;
      let body;
      try {
        ...
        if (!en.value.trim() || !zh.value.trim()) throw new Error('请填写中文和英文说明。');
  ...
  refresh.onclick = () => run(async()=>{await identity();await load();}, '正在读取套餐…');
  run(async()=>{await identity();await load();}, '正在读取套餐…');
}
