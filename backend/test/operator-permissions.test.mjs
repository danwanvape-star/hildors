...
test('operator permissions are enforced and version changes revoke sessions',async t=>{
 ...
 const cookie=login.cookie;assert.equal((await call('/admin/packages',{cookie})).status,200);
 ...
 assert.equal((await call('/admin/operators/'+created.data.id,{body:{version:1,permissions:['content.view'],active:true,displayName:'审核员'}})).status,200);
...
test('creator review cannot edit commercial settings; order review cannot change quotes',async t=>{
 ...
 const base=`http://127.0.0.1:${server.address().port}`;
 const login=await fetch(base+'/admin/login',{method:'POST',body:JSON.stringify({username:op.username,password:'test-operator-password'})});const cookie=login.headers.get('set-cookie').split(';')[0];
...
test('in-flight media inspection cannot commit after permission revocation',async t=>{
 ...
 const base=`http://127.0.0.1:${server.address().port}`;
 const login=await fetch(base+'/admin/login',{method:'POST',body:JSON.stringify({username:op.username,password:'test-upload-password'})});const cookie=login.headers.get('set-cookie').split(';')[0];
 const result=fetch(base+`/admin/packages/${item.id}/clips/main/inspect`,{method:'POST',headers:{cookie}});
 await begun;store.updateOperator(op.id,{version:1,permissions:[]},{id:'root'});release({status:'failed',code:'SHOULD_NOT_COMMIT'});
...
test('creator commercial updates preserve review fields and clip publish requires review too',async t=>{
 ...
 const base=`http://127.0.0.1:${server.address().port}`,login=await fetch(base+'/admin/login',{method:'POST',body:JSON.stringify({username:'commercial',password:'test-commercial-password'})});const cookie=login.headers.get('set-cookie').split(';')[0];
