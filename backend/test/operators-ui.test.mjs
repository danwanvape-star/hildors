...
test('required password change hides business controls even with granted permissions',async()=>{
 const {run,context,guarded}=fixture();
 context.api=async()=>({actor:{id:'one',isRoot:false,mustChangePassword:true},permissions:['content.delete']});
 await run('refreshAdminIdentity()');assert.equal(guarded.hidden,true);assert.equal(run("canAdmin('content.delete')"),false);
});
test('self password dialog works without management grant and never retains passwords',async()=>{
 const {run,get,context}=fixture();let path,body,cleaned=false;context.endAdminSession=()=>{cleaned=true;run('clearOperatorManagement()');};
 context.api=async(p,b)=>{if(p==='/admin/me')return {actor:{id:'one',isRoot:false,mustChangePassword:true},permissions:[]};path=p;body={...b};return {authenticated:false};};
 context.showConnection=()=>{};
 await run('refreshAdminIdentity()');run('openOwnPassword()');assert.equal(get('own-password-dialog').open,true);
 get('own-current-password').value='old-test-password';get('own-new-password').value=get('own-confirm-password').value='new-test-password';
 await run('saveOwnPassword({preventDefault(){}})');
 assert.equal(cleaned,true);assert.equal(path,'/admin/me/password');assert.equal(body.currentPassword,'old-test-password');assert.equal(get('own-current-password').value,'');assert.equal(get('own-new-password').value,'');assert.equal(get('own-password-dialog').open,false);
 assert.equal(run('adminActor'),null);
});

test('full logout invalidates delayed catalog responses after password change',async()=>{
 const {run,context,get}=fixture();
 const source=readFileSync(new URL('../public/app.js',import.meta.url),'utf8');
 const end=source.match(/function endAdminSession\(\) \{[^\n]+/)[0];
 const refresh=source.slice(source.indexOf('async function refresh()'),source.indexOf("$('login').onsubmit"));
 let resolve;context.api=()=>new Promise(done=>{resolve=done;});
 for(const name of ['clearAudit','clearOrderDetail','clearCreatorManagement','clearPreviews','showConnection','refreshTagFilter','render','showView'])context[name]=()=>{};
 run('let epoch=0,items=[],orders=[],creators=[],layoutState=null,reviewing=null,selectedItemId=null,contentTags=[],activeView="content";');
 run(end);run(refresh);
 // Use one shared delayed response for both catalog requests.
 let release;const pending=new Promise(done=>{release=done;});context.api=()=>pending;
 const work=run('refresh()');run('endAdminSession()');release({items:[{id:'private'}]});await work;
 assert.equal(run('epoch'),1);assert.equal(run('items.length'),0);assert.equal(get('cards').children.length,0);
});
